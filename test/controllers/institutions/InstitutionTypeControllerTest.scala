...
class InstitutionTypeControllerTest extends PlaySpec with GuiceOneAppPerTest  with Injecting {

  val entity =InstitutionType("I01","University",None)
  ...
  "EntityController" should {
    ...
    "Create Entity" in {
      ...
      val request = route(app, FakeRequest(POST, "/institutions/type/create")
    ...
    "Read Entity " in {
      ...
      val request = route(app, FakeRequest(GET, "/institutions/type/get/$id" + entity.id)
        .withHeaders(AUTHORIZATION -> token)
      ).get
      status(request) mustBe OK
      contentType(request) mustBe Some("application/json")
    ...
    "Get Entities" in {
      val request = route(app, FakeRequest(GET, "/institutions/type/all")
        .withHeaders(AUTHORIZATION -> token)
      ).get
      status(request) mustBe OK
      contentType(request) mustBe Some("application/json")
    ...
    "Update Entity" in {
      ...
      val request = route(app, FakeRequest(POST, "/institutions/type/update")
    ...
    "Delete Entities" in {
      val request = route(app, FakeRequest(POST, "/institutions/type/delete")
        .withJsonBody(Json.toJson(entity))
        .withHeaders(AUTHORIZATION -> token)
      ).get
      status(request) mustBe OK
