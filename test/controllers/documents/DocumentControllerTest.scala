...
class DocumentControllerTest extends PlaySpec with GuiceOneAppPerTest  with Injecting {
  ...
  val entity =Document("Malusi","htpp://www.google.com/drive","Year 2017","National Certification","N",LocalDateTime.now(),"yes", "")
  ...
  "EntityController" should {
    ...
    "Read Entity " in {
      ...
      val request = route(app, FakeRequest(GET, "/documents/get/$email" + entity.documentId)
        .withHeaders(AUTHORIZATION -> token)
      ).get
      status(request) mustBe OK
      contentType(request) mustBe Some("application/json")
