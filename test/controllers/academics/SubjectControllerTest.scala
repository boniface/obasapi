...
class SubjectControllerTest extends PlaySpec with GuiceOneAppPerTest  with Injecting {
  ...
  "EntityController" should {
    ...
    "Create Entity" in {
      ...
      val request = route(app, FakeRequest(POST, "/subjects/university/create")
    ...
    "Read Entity " in {
      ...
      val request = route(app, FakeRequest(GET, "/subjects/university/get/$courseCode" + entity.courseCode)
        .withHeaders(AUTHORIZATION -> token)
      ).get
      status(request) mustBe OK
      contentType(request) mustBe Some("application/json")
    ...
    "Get Entities" in {
      val request = route(app, FakeRequest(GET, "/subjects/university/all")
        .withHeaders(AUTHORIZATION -> token)
      ).get
      status(request) mustBe OK
      contentType(request) mustBe Some("application/json")
    ...
    "Update Entity" in {
      ...
      val request = route(app, FakeRequest(POST, "/subjects/university/update")
    ...
    "Delete Entities" in {
      val request = route(app, FakeRequest(POST, "/subjects/university/delete")
        .withJsonBody(Json.toJson(entity))
        .withHeaders(AUTHORIZATION -> token)
      ).get
      status(request) mustBe OK
