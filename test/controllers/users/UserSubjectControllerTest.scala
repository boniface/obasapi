...
class UserSubjectControllerTest extends PlaySpec with GuiceOneAppPerTest  with Injecting {

  val entity =UserSubject("1","Biographics","Life Science",55.6)
  ...
  "EntityController" should {
    ...
    "Read Entity " in {
      ...
      val request = route(app, FakeRequest(GET, "/users/subjects/get/$userSubjectId" + entity.subjectId)
        .withHeaders(AUTHORIZATION -> token)
      ).get
      status(request) mustBe OK
      contentType(request) mustBe Some("application/json")
