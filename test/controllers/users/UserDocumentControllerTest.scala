...
class UserDocumentControllerTest extends PlaySpec with GuiceOneAppPerTest  with Injecting {

  val entity =UserDocument("1","i though we suppose to link the document id Here")
  ...
  "EntityController" should {
    ...
    "Read Entity " in {
      ...
      val request = route(app, FakeRequest(GET, "/users/documents/get/$userDocumentsId" + entity.userId)
        .withHeaders(AUTHORIZATION -> token)
      ).get
      status(request) mustBe OK
      contentType(request) mustBe Some("application/json")
