...
class UserApplicationInstitutionController @Inject()
(cc: ControllerComponents, api: ApiResponse) extends AbstractController(cc) with Logging {

  type DomainObject = UserApplicationInstitution
  def className: String = "UserApplicationInstitutionController"
  override val logger: Logger = Logger(className)
  def domainService: UserApplicationInstitutionService = UserApplicationInstitutionService.apply
