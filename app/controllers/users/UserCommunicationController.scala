...
(cc: ControllerComponents, api: ApiResponse) extends AbstractController(cc){
  ...
  def create: Action[JsValue] = Action.async(parse.json) {
    implicit request: Request[JsValue] =>
      ...
      entity match {
        case Right(value) =>
          val response: Future[Option[UserCommunication]] = for {
            results: Option[UserCommunication] <- domainService.saveEntity(value)
          } yield results
          api.requestResponse[Option[UserCommunication]](response, className)
  ...
  def getUserCommunicationById(communicationId: String): Action[AnyContent] = Action.async {
    implicit request: Request[AnyContent] =>
      val response: Future[Option[DomainObject]] = for {
        results <- domainService.getEntity(communicationId)
      } yield results
