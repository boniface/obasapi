...
(cc: ControllerComponents, api: ApiResponse) extends AbstractController(cc){
  ...
  def create: Action[JsValue] = Action.async(parse.json) {
    implicit request: Request[JsValue] =>
      ...
      entity match {
        case Right(value) =>
          val response: Future[Option[UserRelative]] = for {
            results: Option[UserRelative] <- domainService.saveEntity(value)
          } yield results
          api.requestResponse[Option[UserRelative]](response, className)
