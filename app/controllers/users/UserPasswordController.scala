...
(cc: ControllerComponents, api: ApiResponse) extends AbstractController(cc){
  ...
  def create: Action[JsValue] = Action.async(parse.json) {
    implicit request: Request[JsValue] =>
      ...
      entity match {
        case Right(value) =>
          val response: Future[Option[UserPassword]] = for {
            results: Option[UserPassword] <- domainService.saveEntity(value)
          } yield results
          api.requestResponse[Option[UserPassword]](response, className)
  ...
  def getAllUserPassword: Action[AnyContent] = Action.async {
    implicit request: Request[AnyContent] =>
  ...
  def deleteUserPassword: Action[JsValue] = Action.async(parse.json) {
    implicit request: Request[JsValue] =>
      val entity = Json.fromJson[DomainObject](request.body).asEither
      entity match {
        case Right(value) =>
