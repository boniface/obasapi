...
class UserApplicationCourseController @Inject()
(cc: ControllerComponents, api: ApiResponse) extends AbstractController(cc) with Logging {

  type DomainObject = UserApplicationCourse
  def className: String = "UserApplicationCourseController"
  override val logger: Logger = Logger(className)
  def domainService: UserApplicationCourseService = UserApplicationCourseService.apply
