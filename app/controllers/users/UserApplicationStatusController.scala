...
(cc: ControllerComponents, api: ApiResponse) extends AbstractController(cc) with Logging {
  ...
  def read(applicationId: String, statusId: String): Action[AnyContent] = Action.async {
    implicit request: Request[AnyContent] =>
      logger.info("Retrieve by applicationId: " + applicationId + " and statusId: " + statusId)
      println("Retrieve by applicationId: " + applicationId + " and statusId: " + statusId)
      val response: Future[Option[DomainObject]] = for {
        results <- domainService.getEntity(applicationId, statusId)
      } yield results
      api.requestResponse[Option[DomainObject]](response, className)
  }
  ...
  def getEntitiesForApplication(applicationId: String): Action[AnyContent] = Action.async {
    implicit request: Request[AnyContent] =>
      logger.info("Retrieve by userId: " + applicationId)
      println("Retrieve by userId: " + applicationId)
