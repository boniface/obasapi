...
class InstitutionTypeController @Inject()
(cc: ControllerComponents, api: ApiResponse) extends AbstractController(cc) with Logging {

  type DomainObject = InstitutionType

  def className: String = "InstitutionLocationController"
  override val logger: Logger = Logger(className)
  def domainService: InstitutionTypeService = InstitutionTypeService.apply
    ...
    implicit request: Request[JsValue] =>
      ...
      entity match {
        case Right(value) =>
          ...
          logger.info("Saving institution type: " + copy)
